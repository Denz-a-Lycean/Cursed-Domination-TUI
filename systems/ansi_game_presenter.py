...
class AnsiGamePresenter:
    ...
    def domain_expansion_animation(
        ...
        tag, glyph_set = presets.get(technique_key, presets["Seeker"])
        ...
        while True:
            ...
            inner_w = self.window.width - 4
            px = self.ox + 2
            pct = int((remaining / duration_seconds) * 100)
            g = glyph_set[int(elapsed * 4) % len(glyph_set)]
            msgs = [
                "Draining cursed energy...",
            ...
            if parsed:
                ...
                buf = self._battle_panel_buffer(
                    ...
                    parsed=parsed,
                    message=msg,
                    footer="Domain active. Enemy turn begins when the barrier collapses.",
                    side_lines=[
                        tag,
                        f"TIME LEFT  {remaining:05.1f}s",
                        f"BARRIER   {pct}% STABLE",
                    ],
                    side_title="DOMAIN ACTIVE",
                    frame=int(elapsed * 6),
                )
                title_line = f"{Colors.BRIGHT_RED}{domain_name.upper()}{RESET}"
                timer_line = (
                    f"{FG_LABEL}BARRIER{RESET} {barrier_bar} "
                    f"{Colors.WHITE}{remaining:05.1f}s{RESET}"
                )
                duel_line = (
                    f"{Colors.WHITE}{attacker_name}{RESET} seals "
                    f"{Colors.WHITE}{defender_name}{RESET} inside the domain."
                )
                buf += f"{ESC}{self.oy + 12};{px}H{_center_ansi_line(title_line, inner_w)}"
                buf += f"{ESC}{self.oy + 13};{px}H{_center_ansi_line(timer_line, inner_w)}"
                buf += f"{ESC}{self.oy + 14};{px}H{_center_ansi_line(duel_line, inner_w)}"
            else:
                buf = self._frame_buffer(title)
                row = 5
                lines = [
                    color_text("DOMAIN ACTIVE", Colors.BRIGHT_RED),
                    domain_name,
                    "",
                    tag,
                    f"Inside: {attacker_name}  vs  {defender_name}",
                    "",
                    f"{g * 20}  {pct}%  {g * 20}",
                    f"TIME LEFT: {remaining:05.1f}s",
                    msgs[int(elapsed * 2) % len(msgs)],
                ]
                for ln in lines:
                    buf += f"{ESC}{self.oy + row};{self.ox + 2}H{_clamp(ln, inner_w)}{RESET}"
                    row += 1
                if st:
                    row += 1
                    for sl in st:
            ...
            while time.monotonic() < deadline:
                ...
                if nb is None:
                    time.sleep(0.02)
                    continue
                # swallow stray input during cinematic timer
                break
